...
export async function POST(req: Request) {
  try {
    ...
    if (body.action === "start") {
      if (!body.personId) return NextResponse.json({ ok: false, error: "missing personId" }, { status: 400 });
...
export async function GET() {
  const sessions = await prisma.workSession.findMany({ where: { active: true }, include: { person: true } });
