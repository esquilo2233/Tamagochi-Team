...
export default function PetCompanion({
  ...
  useEffect(() => {
    ...
    // Apenas atualizar o tempo, não buscar moedas continuamente
    async function updateTime() {
      try {
        const sRes = await fetch(`/api/companion?code=${code}`);
        const sessions = await sRes.json();
        if (sessions && sessions.length > 0) {
          // Mostrar tempo decorrido, mas moedas só no final
        }
      } catch (e) {}
    }
  ...
  async function handleStop() {
    ...
    try {
      ...
      // Mostrar moedas ganhas no final
      if (data.ok && data.session) {
        const totalCoins = data.session.coinsEarned || 0;
        const minutes = Math.floor(timeSeconds / 60);
        alert(
          `🎉 Companhia terminada!\n\n⏱️ Tempo: ${minutes} minutos\n💰 Moedas ganhas: ${totalCoins}`,
        );
      }
    } catch (e) {
      console.error("Erro ao parar companhia", e);
    }
    onClose();
  }
